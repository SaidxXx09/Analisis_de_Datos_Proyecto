...
def cargar_stores():
    df = pl.read_csv("data/datasets/stores.csv")
    df.write_parquet("data/processed/stores.parquet")
    print(f"stores cargado: {df.shape}")


def diagnosticar_stores():
    df = pl.read_parquet("data/processed/stores.parquet")
    ...
    reporte["INFORMACION_STORES"] = info_stores
